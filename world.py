...
class World:
    def __init__(self, env, randomise_observations=False, randomise_policy=False, shiny=None):
        ...
        # If it's no filename, it's assumed to be an environment dictionary
        ...
        # If shiny data is provided: initialise shiny properties
            ...
            while len(self.shiny['locations']) < self.shiny['n']:
                ...
                too_close = [dist_matrix[new, existing] < np.max(dist_matrix) / self.shiny['n'] for existing in
                             self.shiny['locations']]
            ...
            not_shiny = [observation for observation in range(self.n_observations) if
                         observation not in self.shiny['objects']]
    ...
    def get_reward(self, reward_locations):
        ...
        for reward_location in reward_locations:
            ...
            for action in new_locations[reward_location]['actions']:
                action['probability'] = action['probability'] / total_probability if total_probability > 0 else \
                    action['probability']
